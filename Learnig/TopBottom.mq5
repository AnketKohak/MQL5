...
int OnInit()
  {
   ...
   handleBollinger = iBands(_Symbol,PERIOD_M1, 20, 2.0, 0, PRICE_CLOSE);
...
void OnTick()
  {
   double upperBollingerBand, lowerBollingerBand, currentWPR, indBuffer[];
   ...
   CopyBuffer(handleWPR, 0, 0, 1, indBuffer);
...
void UpdateProfitDisplay(string labelName, double profit){
  color textColor = profit >= 0 ? clrLime : clrRed;
    string text = DoubleToString(profit, 2);

    ObjectSetString(0,labelName + "Label",OBJPROP_TEXT,text);
    ObjectSetInteger(0,labelName + "Label",OBJPROP_COLOR,textColor);
