...
input double triggerPoint = 3.00;
input double trailStopLoss = 100;
...
void OnTick()
  {
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
     {
      ord.SelectByIndex(i);
      if(ord.OrderType() ==  ORDER_TYPE_BUY_LIMIT && ord.Symbol() == _Symbol && ord.Magic() == InpMagic)
        {
         BuyTotal++;
        }
      if(ord.OrderType() ==  ORDER_TYPE_SELL_LIMIT && ord.Symbol() == _Symbol && ord.Magic() == InpMagic)
        {
         SellTotal++;
        }
     }
            ...
            if(pos.PositionType() == POSITION_TYPE_BUY)
              {
               ...
               if(profit > triggerPoint)
                 {
                  sl = bid - (trailStopLoss * _Point);
...
void executeBuy(double entry)
  {
   ...
   trade.BuyLimit(lots, entry, _Symbol, sl, 0, ORDER_TIME_SPECIFIED, expiration);
  }
...
void executeSell(double entry)
  {
   ...
   double sl = entry + stoploss * _Point;
   datetime expiration = iTime(_Symbol, Timeframe, 0) + ExpirationBars * PeriodSeconds(Timeframe);
   trade.SellLimit(lots, entry, _Symbol, sl, 0, ORDER_TIME_SPECIFIED, expiration);
  }
